# -*- coding: utf-8 -*-
"""
V24 strong recent-baseline benchmark under the frozen V15 stream/FedAvg protocol.

This file is intentionally baseline-only in its default ``--policy_subset recent``
run.  It does NOT rerun MC-PrivGLR/Ours.

Frozen V24 baselines
--------------------
1) Adaptive-OSMD-NativeGrad-OGPM (JMLR 2025, source-adapted)
   The official lr_set_fun and Adaptive_OSMD_Expert are executed directly.
   Selection uses the OSMD sampling distribution.  After a selected client
   trains, its source-native squared gradient-norm feedback is bounded using the
   public common gradient clipping constant and only that scalar is released by
   the official epsilon-LDP Optimal-GPM channel.

2) FEROMA-Cal-OGPM-EMA (ICLR 2026, source-adapted)
   Each client locally forms an extended latent [mean,std] profile.  The stable
   calibration prefix fixes a client-local reference and q95 Euclidean-distance
   scale.  After calibration, one bounded profile-shift scalar is released by
   the official epsilon-LDP Optimal-GPM channel, the server applies a fixed
   EMA(alpha=0.20), and Top-B clients are scheduled.

Both baselines use the same dataset partition, drift/onset stream, B, warm-up
checkpoint, local SGD, FedAvg, evaluation schedule, and formal seeds as V15.
Their source-specific observation/feedback routes are preserved rather than
forcing both methods onto the same loss proxy.

Privacy statement
-----------------
Every released scalar separately satisfies event-level epsilon-LDP under the
common Optimal-GPM mechanism.  Repeated releases compose over time; this file
does not claim trajectory-level privacy cost epsilon.
"""
...
from v24_strong_baseline_adapters import (
    AdaptiveOSMDNativeSignalLDPAdapter,
    FeromaCalibratedProfileOGPMEMAAdapter,
)
...
VERSION = "V24_strong_baselines_AdaptiveOSMD_NativeGrad_plus_FEROMA_Cal_OGPM_EMA_frozen"
...
def _resolve_cinic10_root(root: str, explicit_path: str = "") -> Path:
    """Resolve an extracted CINIC-10 directory containing train/valid/test."""
    ...
    for candidate in candidates:
        ...
        if (
            ...
            and (candidate / "valid").is_dir()
            and (candidate / "test").is_dir()
        ):
            return candidate.resolve()
    raise FileNotFoundError(
        "Could not locate extracted CINIC-10. Expected train/valid/test below one "
        "of: " + ", ".join(checked) + ". Extract CINIC-10 first or pass "
        "--cinic10_path explicitly."
    )


def _decode_imagefolder_uint8(
    split_dir: Path,
) -> Tuple[np.ndarray, np.ndarray, Dict[str, int]]:
    """Decode one CINIC-10 ImageFolder split to contiguous 32x32 RGB uint8 arrays."""
    ...
    print(f"Decoding CINIC-10 split {split_dir.name}: {n} images ...", flush=True)
    for index, (image_path, target) in enumerate(dataset.samples):
        ...
        if (index + 1) % 10000 == 0 or index + 1 == n:
            print(f"  {split_dir.name}: {index + 1}/{n}", flush=True)

    return images, labels, dict(dataset.class_to_idx)
...
def load_cinic10(
    ...
    """
    Load CINIC-10 train/test into the unchanged array-based V15/V24 pipeline.

    The official validation split is deliberately left unused.  The decoded
    arrays are cached only to avoid repeatedly opening 180k image files.
    """
    dataset_root = _resolve_cinic10_root(args.root, args.cinic10_path)
    cache_dir = Path(args.root) / "cinic10_processed"
    cache_dir.mkdir(parents=True, exist_ok=True)

    paths = {
        "train_x": cache_dir / "train_x_uint8.npy",
        "train_y": cache_dir / "train_y_int64.npy",
        "test_x": cache_dir / "test_x_uint8.npy",
        "test_y": cache_dir / "test_y_int64.npy",
        "meta": cache_dir / "meta.json",
    }

    cache_ready = all(
        paths[key].is_file()
        for key in ("train_x", "train_y", "test_x", "test_y", "meta")
    )
    if cache_ready:
        meta = json.loads(paths["meta"].read_text(encoding="utf-8"))
        if meta.get("dataset_root") == str(dataset_root):
            train_x = np.load(paths["train_x"], mmap_mode="r")
            train_y = np.load(paths["train_y"], mmap_mode="r")
            test_x = np.load(paths["test_x"], mmap_mode="r")
            test_y = np.load(paths["test_y"], mmap_mode="r")
            print(
                f"Loaded CINIC-10 array cache: train={train_y.size}, "
                f"test={test_y.size}, root={dataset_root}",
                flush=True,
            )
            return train_x, train_y, test_x, test_y, 10

    train_x, train_y, train_classes = _decode_imagefolder_uint8(
        dataset_root / "train"
    )
    test_x, test_y, test_classes = _decode_imagefolder_uint8(
        dataset_root / "test"
    )
    if train_classes != test_classes:
        raise RuntimeError(
            "CINIC-10 class mapping differs between train/test: "
            f"{train_classes} vs {test_classes}"
        )
    if len(train_classes) != 10:
        raise RuntimeError(
            f"Expected 10 CINIC-10 classes, found {len(train_classes)}: "
            f"{train_classes}"
        )
    ...
    valid_count = len(ImageFolder(str(dataset_root / "valid")))
    paths["meta"].write_text(
        json.dumps(
            {
                "dataset_root": str(dataset_root),
                "train_count": int(train_y.size),
                "valid_count": int(valid_count),
                "test_count": int(test_y.size),
                "class_to_idx": train_classes,
                "normalization": "frozen V15 CIFAR-10 mean/std",
            },
            indent=2,
            sort_keys=True,
        ),
    ...
    print(
        f"Built CINIC-10 array cache under {cache_dir.resolve()} | "
        f"train={train_y.size}, valid(unused)={valid_count}, test={test_y.size}",
        flush=True,
    )
    return train_x, train_y, test_x, test_y, 10
...
@torch.no_grad()
def monitor_feroma_latent_profiles(
    model: nn.Module,
    t: int,
    plan: StreamPlan,
    images: np.ndarray,
    ...
    """Client-local FEROMA extended latent descriptor [mean, std].

    FEROMA's public source exposes latent-space mean and std descriptors.  V24
    extracts the input to the final ``fc`` layer and concatenates the per-window
    latent mean and standard deviation.  The raw vector never leaves the client
    side of the simulation; only a later bounded profile-distance scalar is
    released through OGPM.
    """
    del labels
    model.eval()
    n = plan.monitor_indices.shape[1]
    batch_per_client = plan.monitor_indices.shape[2]
    profiles = None
    captured = []

    def _capture_fc_input(module, inputs):
        del module
        captured.append(inputs[0].detach())

    handle = model.fc.register_forward_pre_hook(_capture_fc_input)
    try:
        for start in range(0, n, chunk_clients):
            stop = min(n, start + chunk_clients)
            idx = plan.monitor_indices[t, start:stop].reshape(-1)
            probs = np.repeat(
                plan.drift_probability[t, start:stop], batch_per_client
            )
            rotate = plan.monitor_rotate_u[t, start:stop].reshape(-1) < probs
            x = images_to_device(images[idx], device, rotate)
            captured.clear()
            ...
            if len(captured) != 1:
                raise RuntimeError(
                    "FEROMA latent hook did not capture exactly one fc input"
                )
            feat = captured[0].float().reshape(
                stop - start, batch_per_client, -1
            )
            mean = feat.mean(dim=1)
            std = feat.std(dim=1, unbiased=False)
            chunk_profile = torch.cat([mean, std], dim=1).cpu().numpy().astype(
                np.float64
            )
            if profiles is None:
                profiles = np.empty(
                    (n, chunk_profile.shape[1]), dtype=np.float64
                )
            profiles[start:stop] = chunk_profile
    finally:
        handle.remove()
    if profiles is None:
        raise RuntimeError("FEROMA profile extraction produced no clients")
    return profiles
...
def adapt_one_fedavg_round(
    ...
    native_grad_sq_unit: List[float] = []
    ...
    for client in selected:
        ...
        for local_step in range(args.local_steps):
            ...
            if args.grad_clip > 0:
                ...
                # Re-use this value for the native Adaptive-OSMD feedback
                # instead of re-summing the post-clipping tensors.  This also
                # lets us handle AMP overflow deterministically: GradScaler
                # skips an optimizer step when the unscaled gradients are
                # non-finite, so the effective update for that step is zero.
                total_grad_norm = torch.nn.utils.clip_grad_norm_(
                    local_model.parameters(), args.grad_clip
                )
            elif local_step == 0:
                raise RuntimeError(
                    "Adaptive-OSMD native-gradient LDP feedback requires "
                    "a positive public --grad_clip bound"
                )
            else:
                total_grad_norm = torch.tensor(0.0, device=device)

            # Source-faithful Adaptive-OSMD feedback uses one selected-client
            # gradient norm per communication round.  The source is FedSGD;
            # under our fixed two-step FedAvg protocol, the FIRST local gradient
            # is the closest source-equivalent statistic.  The public clipping
            # bound C guarantees (min(||g||, C)/C)^2 in [0,1].
            # If AMP produced a non-finite first-step gradient, GradScaler will
            # skip that optimizer step; encode its effective update magnitude as
            # zero rather than passing NaN into the official OGPM implementation.
            if local_step == 0:
                raw_norm = float(total_grad_norm.detach().float().cpu())
                if np.isfinite(raw_norm):
                    clipped_norm = min(max(raw_norm, 0.0), float(args.grad_clip))
                    native_unit = (clipped_norm / float(args.grad_clip)) ** 2
                else:
                    native_unit = 0.0
                native_grad_sq_unit.append(float(np.clip(native_unit, 0.0, 1.0)))
    ...
    return {
        ...
        "native_grad_sq_unit": np.asarray(native_grad_sq_unit, dtype=np.float64),
        "native_ogpm_uniforms": np.asarray(
            plan.ogpm_uniforms[t, selected], dtype=np.float64
        ),
    }
...
class OnlineController:
    ...
    def initialize_published_baseline(
        ...
        elif self.kind == "acs2025_private":
            self.published_adapter = AdaptiveOSMDNativeSignalLDPAdapter(
                self.args.official_acs_root,
                self.args.official_acs_commit,
                self.args.num_clients,
                self.args.budget_b,
                self.args.report_levels,
                client_sizes,
                self.args.horizon - self.args.calib_end,
                int(stream_seed) + 31013,
                alpha=self.args.acs_alpha,
            )
        elif self.kind == "feroma2026_private":
            self.published_adapter = FeromaCalibratedProfileOGPMEMAAdapter(
                self.args.official_feroma_root,
                self.args.num_clients,
                self.args.budget_b,
                self.args.report_levels,
                calibration_quantile=self.args.feroma_calibration_quantile,
                ema_alpha=self.args.feroma_ema_alpha,
            )
    ...
    def observe_training_feedback(
        ...
        if self.kind == "oort_private":
            ...
            if self.published_adapter is None or self.epsilon is None:
                raise RuntimeError(
                    "Adaptive-OSMD native-gradient OGPM adapter is not initialized"
                )
            native = np.asarray(
                feedback["native_grad_sq_unit"], dtype=np.float64
            )
            uniforms = np.asarray(
                feedback["native_ogpm_uniforms"], dtype=np.float64
            )
            private_bins = ogpm_report_quantized(
                native,
                float(self.epsilon),
                self.args.report_levels,
                uniforms,
            )
            self.published_adapter.observe_private_bins(selected, private_bins)
    ...
    def select(
        ...
        if t < self.args.calib_end:
            ...
            if self.published_adapter is None:
                raise RuntimeError("Adaptive-OSMD-LDP adapter was not initialized")
            ids, scores = self.published_adapter.select(tie, common_random)
        elif self.kind == "feroma2026_private":
            if self.published_adapter is None:
                raise RuntimeError("FEROMA-Cal-OGPM-EMA adapter was not initialized")
            ids, scores = self.published_adapter.select(
                np.asarray(reports, dtype=np.int64), tie, common_random,
            )
        elif self.kind == "topb":
...
def policy_specs(args) -> List[Tuple[str, str, Optional[float]]]:
    ...
    for epsilon in args.epsilons:
        ...
        all_specs.extend([
            ...
            (f"adaptive_osmd2025_nativegrad_ogpm_{label}", "acs2025_private", epsilon),
            (f"feroma2026_calibrated_profile_ogpm_ema_{label}", "feroma2026_private", epsilon),
    ...
    if "paper" in requested:
        return [
            ...
            if spec[1] in {
                ...
                "hics2024", "fedgcs_private", "acs2025_private",
                "feroma2026_private", "mcglr_top2",
            }
        ]
    if "recent" in requested:
        return [
            spec for spec in all_specs
            if spec[1] in {"acs2025_private", "feroma2026_private"}
        ]
    if "recent_plus_ours" in requested:
        return [
            spec for spec in all_specs
            if spec[1] in {"acs2025_private", "feroma2026_private", "mcglr_top2"}
        ]
...
def policy_provenance(kind: str) -> Tuple[str, str]:
    ...
    if kind == "acs2025_private":
        return (
            "source-adapted+official-OSMD-primitives+native-gradient-OGPM-feedback+V24-frozen",
            "JMLR 2025: exact lr_set_fun/Adaptive_OSMD_Expert; source native selected-client "
            "gradient-norm-squared feedback is clipped/normalized locally then OGPM-privatized; "
            "fixed-B distinct-client PPS sampling",
        )
    if kind == "feroma2026_private":
        return (
            "source-adapted+official-FEROMA-profile-principle+calibration-scaling+OGPM+EMA+V24-frozen",
            "ICLR 2026 FEROMA: extended latent mean+std profile, Euclidean shift, per-client "
            "calibration-only q95 scaling, one scalar OGPM report, fixed EMA(0.20), fixed-B Top-B; "
            "native all-client personalized aggregation is not claimed",
        )
...
def build_or_load_calibration_report_cache(
    ...
    path = cache_dir / (
        f"v24_calibration_f{fraction_tag:04d}_s{seed}_{signature}.npz"
    )
...
def run_one_policy(
    ...
    for t in range(args.horizon):
        ...
        if kind == "acs2025_private":
            # Source-faithful Adaptive-OSMD chooses from its learned sampling
            # distribution BEFORE seeing this round's native feedback.  No
            # all-client pre-selection monitoring signal is supplied.
        ...
        elif kind == "feroma2026_private":
            if controller.published_adapter is None:
                raise RuntimeError("FEROMA-Cal-OGPM-EMA adapter was not initialized")
            clock = time.perf_counter()
            profiles = monitor_feroma_latent_profiles(
                model, t, plan, train_x, train_y, device,
                args.monitor_chunk_clients, args.amp,
            )
            monitoring_seconds += time.perf_counter() - clock
            if t < args.calib_end:
                # Local-only calibration.  Raw descriptors and q_i are not
                # released; no model adaptation occurs in the frozen protocol.
                controller.published_adapter.record_calibration_profiles(profiles)
                clock = time.perf_counter()
                selected, scores = controller.select(
                    t, None, plan.ties[t], plan.common_random[t], active=None,
                )
                ...
                local_shift = controller.published_adapter.bounded_profile_shift(
                    profiles
                )
                reports = ogpm_report_quantized(
                    local_shift, float(epsilon), args.report_levels,
                    plan.ogpm_uniforms[t],
                )
                clock = time.perf_counter()
                selected, scores = controller.select(
            ...
            curve_rows.append({
                ...
                "monitor_report_bits": int(
                    0 if kind in {"random", "oracle"}
                        ...
                        adaptation_rounds * args.budget_b * report_bits
                        if kind == "acs2025_private"
                        else (
                            adaptation_rounds * args.num_clients * report_bits
                            if kind == "feroma2026_private"
                            else (
                                (t + 1)
                                * min(
    ...
    summary = {
        ...
        "final_monitor_report_bits": int(
            0 if kind in {"random", "oracle"}
                ...
                (args.horizon if args.adapt_during_calibration else args.horizon - args.calib_end) * args.budget_b * report_bits
                if kind == "acs2025_private"
                else (
                    (args.horizon if args.adapt_during_calibration else args.horizon - args.calib_end) * args.num_clients * report_bits
                    if kind == "feroma2026_private"
                    else (
                        args.horizon
                        * min(
...
def paired_comparisons(primary: pd.DataFrame, args) -> pd.DataFrame:
    ...
    baselines = [
        ...
        f"adaptive_osmd2025_nativegrad_ogpm_{label}",
        f"feroma2026_calibrated_profile_ogpm_ema_{label}",
    ]
...
def predeclared_pretest_gate(primary: pd.DataFrame, args) -> pd.DataFrame:
    ...
    for epsilon in args.epsilons:
        ...
        rows.append({
            ...
            "passed": passed,
            "n_pairs": n_pairs,
            "loss_diff_v21_minus_ref": float(loss_diff.mean()),
            "balanced_acc_diff_v21_minus_ref": float(balanced_diff.mean()),
            "stable_acc_diff_v21_minus_ref": float(stable_diff.mean()),
            "loss_win_count": wins,
            "minimum_loss_wins": minimum_wins,
            "criterion": criterion,
        })
    ...
    if gate.empty:
        return pd.DataFrame(columns=[
            ...
            "loss_diff_v21_minus_ref", "balanced_acc_diff_v21_minus_ref",
            "stable_acc_diff_v21_minus_ref", "loss_win_count",
            "minimum_loss_wins", "criterion", "overall_pass", "decision",
        ])
    required = gate[gate.required]
    overall_pass = bool((not required.empty) and required.passed.all())
...
def scientific_config(args) -> dict:
    ...
    config.update({
        ...
        "published_baselines": {
            ...
            "acs2025_repo": "https://github.com/boxinz17/FL-Client-Sampling",
            "acs2025_expected_commit": args.official_acs_commit,
            "acs2025_algorithms_sha256": file_sha256(
                Path(args.official_acs_root) / "Real_Data" / "algorithms_cv.py"
            ),
            "acs2025_interface": (
                "V24 strong/frozen: exact source lr_set_fun and Adaptive_OSMD_Expert; "
                "source a_i=lambda_i^2||g_i||^2 retained using selected-client first-step "
                "gradient norm after common clipping, normalized by grad_clip^2 and privatized "
                "through official OGPM; fixed-B distinct-client PPS sampling"
            ),
            "feroma2026_source": (
                "FEROMA, ICLR 2026; official source-adapted profile-guided baseline"
            ),
            "feroma2026_interface": (
                "extended latent mean+std descriptor and Euclidean profile-distance principle from official source; "
                "per-client q95 scale uses calibration prefix only; one bounded local shift scalar is "
                "released through official OGPM; server applies fixed EMA alpha=0.20; fixed-B scheduling "
                "replaces native all-client participation"
            ),
            "feroma2026_calibration_quantile": args.feroma_calibration_quantile,
            "feroma2026_ema_alpha": args.feroma_ema_alpha,
            "acs2025_native_gradient_public_clip": args.grad_clip,
        },
        "privacy_unit": ("Adaptive-OSMD: one selected-client post-training native-gradient feedback event; "
                         "FEROMA: one post-calibration client profile-shift monitoring event"),
...
def validate_args(args) -> None:
    if str(args.dataset).lower() not in {"cifar10", "cifar100", "cinic10", "femnist"}:
        raise ValueError("--dataset must be cifar10, cifar100, cinic10, or femnist")
    ...
    if args.grad_clip <= 0:
        raise ValueError(
            "V24 Adaptive-OSMD native-gradient feedback requires --grad_clip > 0"
        )
    if not (0.5 <= args.feroma_calibration_quantile < 1.0):
        raise ValueError("--feroma_calibration_quantile must be in [0.5,1)")
    if not (0.0 < args.feroma_ema_alpha <= 1.0):
        raise ValueError("--feroma_ema_alpha must be in (0,1]")
    if args.adapt_during_calibration:
        raise ValueError(
            "V24 strong FEROMA freezes local calibration before adaptation; "
            "do not use --adapt_during_calibration"
        )
    ...
    if not (0 < args.acs_alpha <= 1):
        raise ValueError("--acs_alpha must be in (0,1]")
...
def main(args) -> None:
    ...
    args.num_classes = {"cifar10": 10, "cifar100": 100, "cinic10": 10, "femnist": 62}[args.dataset]
    ...
    print("=" * 112)
    print("V24: strong recent private baselines under the frozen V15 data/FedAvg protocol (Ours not run)")
    ...
    print(
        f"Adaptive-OSMD native feedback: first local gradient, public clip C={args.grad_clip:g}, "
        "selected-client OGPM only"
    )
    print(
        f"FEROMA strong profile: mean+std, Euclidean, q{100*args.feroma_calibration_quantile:g} "
        f"local scale, EMA alpha={args.feroma_ema_alpha:g}"
    )
    print("Privacy: each released scalar is event-level epsilon-LDP; repeated releases compose over time.")
    print(f"Adaptive-OSMD 2025 source root: {Path(args.official_acs_root).resolve()}")
    print(f"FEROMA 2026 source root: {Path(args.official_feroma_root).resolve()}")
    ...
    if args.dataset == "femnist":
        ...
        if args.dataset == "cinic10":
            train_x, train_y, test_x, test_y, loaded_classes = load_cinic10(args)
        else:
    ...
    atomic_to_csv(
        ...
        output / "v24_privacy_channel_diagnostics.csv",
    )
                ...
                if kind in {"poc_private", "oort_private", "hics2024"}
                    ...
                    if kind == "fedgcs_private"
                        ...
                        if kind == "acs2025_private"
                            ...
                            "https://github.com/dariofenoglio98/FEROMA"
                            if kind == "feroma2026_private"
                            else ""
                        )
                    )
    ...
    atomic_to_csv(pd.DataFrame(method_registry), output / "v24_method_registry.csv")
    ...
    for fraction in args.changed_fractions:
        ...
        for seed_index, seed in enumerate(range(args.seed, args.seed + args.num_stream_seeds), start=1):
            curve_path = per_seed_dir / f"v24_curve_f{fraction_tag:04d}_s{seed}.csv"
            summary_path = per_seed_dir / f"v24_summary_f{fraction_tag:04d}_s{seed}.csv"
            decision_path = per_seed_dir / f"v24_decisions_f{fraction_tag:04d}_s{seed}.csv"
            cached = False
            ...
            for policy_index, (name, kind, epsilon) in enumerate(specs, start=1):
                ...
                policy_prefix = f"v24_f{fraction_tag:04d}_s{seed}_{policy_tag}"
    ...
    atomic_to_csv(curves_df, output / "v24_all_curves.csv")
    atomic_to_csv(summary_df, output / "v24_per_seed_summary.csv")
    if args.save_decisions:
        atomic_to_csv(
            pd.concat(all_decisions, ignore_index=True),
            output / "v24_all_decisions.csv",
        )
    ...
    atomic_to_csv(primary_summary, output / "v24_primary_policy_summary.csv")
    atomic_to_csv(pairwise, output / "v24_pairwise_primary.csv")
    leaderboard = development_leaderboard(primary, args)
    atomic_to_csv(leaderboard, output / "v24_formal_leaderboard.csv")
    ...
    report_lines = [
        ...
        "V24只补两个近期强基线，不运行MC-PrivGLR/Ours；结果出来后不再按结果调参。",
        "Adaptive-OSMD: 官方OSMD递归 + 已选客户端原生梯度范数平方反馈；公共grad_clip归一化后再OGPM。",
        f"FEROMA: extended latent mean+std + Euclidean shift + calibration-only q{100*args.feroma_calibration_quantile:g} scale + OGPM + EMA(alpha={args.feroma_ema_alpha:g}) + Top-B。",
        "",
        "隐私口径: 每次实际发布的标量分别满足event-level epsilon-LDP；时间重复发布会组合，不宣称轨迹总预算仍为epsilon。",
        "Adaptive-OSMD只发布已选客户端的私有反馈；FEROMA calibration profile/scale全部留在客户端本地，calibration后才发布每客户端一个私有profile-shift标量。",
        "主指标: changed loss、changed accuracy、balanced accuracy、stable accuracy；AUPRC/Recall@B为身份恢复诊断指标。",
        "",
    ]
    ...
    report_path = output / "V24_强基线冻结版比较说明.txt"
    report_path.write_text("\n".join(report_lines), encoding="utf-8")
    print("\n" + "\n".join(report_lines))
    print("\nKey outputs:")
    for path in [
        report_path,
        output / "v24_primary_policy_summary.csv",
        output / "v24_pairwise_primary.csv",
        output / "v24_method_registry.csv",
        output / "v24_formal_leaderboard.csv",
        output / "v24_per_seed_summary.csv",
        output / "v24_all_curves.csv",
        output / "v24_privacy_channel_diagnostics.csv",
        output / "v24_config.json",
    ]:
        print(" ", path.resolve())
    print("\nV24 frozen strong-baseline leaderboard (lower changed loss is better):")
    display_columns = [
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "V24 fairness-audited recent-baseline comparison under the frozen V15 OGPM/FedAvg protocol"
        ),
    )
    # Environment and output
    parser.add_argument("--root", type=str, default=r".\data")
    parser.add_argument(
        "--dataset", choices=["cifar10", "cifar100", "cinic10", "femnist"],
        default="cifar10",
    )
    ...
    parser.add_argument("--out_dir", type=str, default=r"results\v24_strong_baselines_cifar10")
    ...
    parser.add_argument(
        "--official_acs_root", type=str,
        default=r"baselines_2025_2026\FL-Client-Sampling",
        help="Clone of https://github.com/boxinz17/FL-Client-Sampling.",
    )
    parser.add_argument(
        "--official_acs_commit", type=str,
        default="8d12ac39f10d8ac90a7dfb245c1a77f288ba39c4",
        help="Frozen Adaptive Client Sampling source commit.",
    )
    parser.add_argument(
        "--official_feroma_root", type=str,
        default=r"baselines_2025_2026\FEROMA",
        help="Official ICLR-2026 FEROMA source snapshot: https://github.com/dariofenoglio98/FEROMA.",
    )
    parser.add_argument("--rerun_completed", action="store_true")
    parser.add_argument(
        "--policy_subset", nargs="+", default=["recent"],
        help=(
            "recent, recent_plus_ours, paper, screen, all, or exact policy/controller tokens. "
            "recent contains Adaptive-OSMD-NativeGrad-OGPM (JMLR 2025) and FEROMA-Cal-OGPM-EMA (ICLR 2026). Ours is not included."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--cinic10_path", type=str, default="",
        help=(
            "Optional extracted CINIC-10 directory containing train/valid/test. "
            "If empty, search <root>/cinic10, cinic-10, CINIC-10, and root."
        ),
    )
    parser.add_argument(
    ...
    # Adaptive Client Sampling (JMLR 2025): source alpha=0.4.
    parser.add_argument("--acs_alpha", type=float, default=0.4)

    # V24 FEROMA stronger-but-principled choices are frozen before formal runs.
    parser.add_argument("--feroma_calibration_quantile", type=float, default=0.95)
    parser.add_argument("--feroma_ema_alpha", type=float, default=0.20)
