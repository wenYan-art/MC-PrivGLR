# -*- coding: utf-8 -*-
"""
V20 sensitivity benchmark built on the frozen V15 MC-PrivGLR protocol.
...
VERSION = "V20_MCPrivGLR_sensitivity_v1"
...
def load_femnist_federated(args) -> Tuple[
    ...
    cache_name = (
        f"v20s_writers_n{args.num_clients}_seed{args.data_seed}_"
...
def build_or_load_calibration_report_cache(
    ...
    path = cache_dir / (
        f"v20s_calibration_f{fraction_tag:04d}_s{seed}_{signature}.npz"
    )
...
def predeclared_pretest_gate(primary: pd.DataFrame, args) -> pd.DataFrame:
    ...
    for epsilon in args.epsilons:
        ...
        rows.append({
            ...
            "passed": passed,
            "n_pairs": n_pairs,
            "loss_diff_v20s_minus_ref": float(loss_diff.mean()),
            "balanced_acc_diff_v20s_minus_ref": float(balanced_diff.mean()),
            "stable_acc_diff_v20s_minus_ref": float(stable_diff.mean()),
            "loss_win_count": wins,
            "minimum_loss_wins": minimum_wins,
            "criterion": criterion,
        })
    ...
    if gate.empty:
        return pd.DataFrame(columns=[
            ...
            "loss_diff_v20s_minus_ref", "balanced_acc_diff_v20s_minus_ref",
            "stable_acc_diff_v20s_minus_ref", "loss_win_count",
            "minimum_loss_wins", "criterion", "overall_pass", "decision",
        ])
    required = gate[gate.required]
    overall_pass = bool((not required.empty) and required.passed.all())
...
def validate_args(args) -> None:
    ...
    if (
        ...
        or (mcglr_shifts.size > 1 and np.any(np.diff(mcglr_shifts) <= 0))
    ):
        raise ValueError("--mcglr_shifts must contain one or more strictly increasing values in (0,1)")
...
def main(args) -> None:
    ...
    print("=" * 112)
    print("V20-Sensitivity: frozen MC-PrivGLR versus source-backed published baselines + FedGCS")
    ...
    atomic_to_csv(
        ...
        output / "v20s_privacy_channel_diagnostics.csv",
    )
    ...
    atomic_to_csv(pd.DataFrame(method_registry), output / "v20s_method_registry.csv")
    ...
    for fraction in args.changed_fractions:
        ...
        for seed_index, seed in enumerate(range(args.seed, args.seed + args.num_stream_seeds), start=1):
            curve_path = per_seed_dir / f"v20s_curve_f{fraction_tag:04d}_s{seed}.csv"
            summary_path = per_seed_dir / f"v20s_summary_f{fraction_tag:04d}_s{seed}.csv"
            decision_path = per_seed_dir / f"v20s_decisions_f{fraction_tag:04d}_s{seed}.csv"
            cached = False
            ...
            for policy_index, (name, kind, epsilon) in enumerate(specs, start=1):
                ...
                policy_prefix = f"v20s_f{fraction_tag:04d}_s{seed}_{policy_tag}"
    ...
    atomic_to_csv(curves_df, output / "v20s_all_curves.csv")
    atomic_to_csv(summary_df, output / "v20s_per_seed_summary.csv")
    if args.save_decisions:
        atomic_to_csv(
            pd.concat(all_decisions, ignore_index=True),
            output / "v20s_all_decisions.csv",
        )
    ...
    atomic_to_csv(primary_summary, output / "v20s_primary_policy_summary.csv")
    atomic_to_csv(pairwise, output / "v20s_pairwise_primary.csv")
    leaderboard = development_leaderboard(primary, args)
    atomic_to_csv(leaderboard, output / "v20s_formal_leaderboard.csv")
    ...
    for path in [
        report_path,
        output / "v20s_primary_policy_summary.csv",
        output / "v20s_pairwise_primary.csv",
        output / "v20s_method_registry.csv",
        output / "v20s_formal_leaderboard.csv",
        output / "v20s_per_seed_summary.csv",
        output / "v20s_all_curves.csv",
        output / "v20s_privacy_channel_diagnostics.csv",
        output / "v20s_config.json",
    ]:
        print(" ", path.resolve())
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--out_dir", type=str, default=r"results\v20s_official_cifar10")
    ...
    parser.add_argument(
        ...
        help="Commit recorded in v20s_config.json; verify with git rev-parse HEAD.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--policy_subset", nargs="+", default=["mcglr_top2"],
        help=(
    ...
    parser.add_argument("--async_window", type=int, default=5)
    parser.add_argument("--ramp_windows", type=int, default=5)
    ...
    parser.add_argument("--warmup_rounds", type=int, default=120)
    parser.add_argument("--warmup_clients_per_round", type=int, default=20)
