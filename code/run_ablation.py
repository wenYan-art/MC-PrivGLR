# -*- coding: utf-8 -*-
"""
V20 paper ablation benchmark built on the frozen V15 MC-PrivGLR protocol.
...
Controlled ablation set
-----------------------
All variants use the same dataset, stream, exact OGPM channel, client-specific
calibration, local training, FedAvg aggregation, and evaluation.  They differ
only in how the private GLR evidence is tracked or fused:
1. MC-PrivGLR-top2: full method; mean of the two strongest accumulated tracks.
2. MC-PrivGLR-top1: strongest accumulated track only.
3. MC-PrivGLR-single: one predeclared fixed-shift accumulated track.
4. MC-PrivGLR-instant-top2: top-2 exact-channel evidence from the current
   private report only, with no temporal accumulation.
Random and Oracle may be included as controls but are not method components.
...
VERSION = "V20_MCPrivGLR_paper_ablation_v1"
...
class OnlineController:
    ...
    def select(
        ...
        if t < self.args.calib_end:
            ...
            instant_charts = np.maximum(
                0.0,
                llr - self.args.mcglr_drift,
            )
            if mode == "instant_top2":
                # No-history ablation.  It uses the same calibrated stable law,
                # shifted laws, exact OGPM channel, drift margin and top-2
                # fusion as the full method, but discards all previous-window
                # evidence before scoring the current report.
                self.mcglr_charts = instant_charts
            else:
            ...
            if mode in {"max", "top1"}:
                ...
                # Single-scale ablation.  The public target is fixed before
                # seeing any stream outcome; the closest member of the common
                # six-shift grid is used for every client and every window.
                shift_index = int(np.argmin(np.abs(
                    self.mcglr_emission.shifts
                    - float(self.args.mcglr_single_shift)
                )))
                scores = self.mcglr_charts[:, shift_index]
            elif mode == "instant_top2":
                k = min(2, self.mcglr_charts.shape[1])
                scores = np.mean(
                    np.partition(self.mcglr_charts, -k, axis=1)[:, -k:],
                    axis=1,
...
def policy_specs(args) -> List[Tuple[str, str, Optional[float]]]:
    ...
    if "core_ablation" in requested:
        # Main paper ablation: test fusion and temporal accumulation only.
        # Single-scale is treated separately as a scale-sensitivity diagnostic.
        return [
            spec for spec in all_specs
            if spec[1] in {
                "mcglr_top2", "mcglr_top1", "mcglr_instant_top2",
            }
        ]
    if "fixed_shift" in requested:
        # One predeclared single-scale run. Use --mcglr_single_shift to choose it.
        return [spec for spec in all_specs if spec[1] == "mcglr_single"]
    if "ablation" in requested:
        # Legacy full diagnostic set retained for backward compatibility.
        return [
            spec for spec in all_specs
            if spec[1] in {
                "random", "oracle", "mcglr_top2", "mcglr_top1",
                "mcglr_single", "mcglr_instant_top2",
            }
        ]
...
def policy_provenance(kind: str) -> Tuple[str, str]:
    ...
    if kind == "mcglr_top2":
        return "ours-full", "frozen V15 MC-PrivGLR top-2 fusion"
    if kind == "mcglr_top1":
        return "ours-ablation", "remove consensus fusion; strongest track only"
    if kind == "mcglr_single":
        return "ours-ablation", "remove multiscale tracking; one fixed shift"
    if kind == "mcglr_instant_top2":
        return "ours-ablation", "remove temporal accumulation; current report only"
    if kind.startswith("mcglr_"):
        return "ours-diagnostic", kind
    if kind == "random":
...
def ablation_comparisons(primary: pd.DataFrame, args) -> pd.DataFrame:
    """Paired, seed-wise comparison of the frozen full method and removals.

    Negative full-minus-ablation differences are favourable for losses;
    positive differences are favourable for accuracy and identity metrics.
    Stable and balanced accuracy are retained as safety diagnostics even when
    the paper emphasizes the four adaptation/identification metrics.
    """
    label = epsilon_label(args.primary_epsilon)
    full_name = f"mcglr_top2_ogpm_{label}"
    variants = [
        f"mcglr_top1_ogpm_{label}",
        f"mcglr_single_ogpm_{label}",
        f"mcglr_instant_top2_ogpm_{label}",
    ]
    metrics = [
        "mean_changed_loss_post",
        "mean_changed_acc_post",
    ...
    lower_is_better = {"mean_changed_loss_post"}
    available = set(primary.policy.unique())
    if full_name not in available:
        return pd.DataFrame()
    rows = []
    full = primary[primary.policy == full_name]
    for v, variant in enumerate(variants):
        if variant not in available:
            continue
        ablated = primary[primary.policy == variant]
        for m, metric in enumerate(metrics):
            merged = full[["seed", metric]].merge(
                ablated[["seed", metric]],
                on="seed",
            ...
            if metric in lower_is_better:
                ...
                favourable_mean = bool(np.isfinite(mean) and mean < 0.0)
            else:
                wins = int(np.sum(finite > 0.0))
                favourable_mean = bool(np.isfinite(mean) and mean > 0.0)
            rows.append({
                ...
                "mean_diff_full_minus_ablation": mean,
                "ci95_lo": lo,
                "ci95_hi": hi,
                "full_win_count": wins,
                "n_pairs": int(finite.size),
                "favourable_mean_for_full": favourable_mean,
            })
    return pd.DataFrame(rows)


def ablation_gate(comparisons: pd.DataFrame) -> pd.DataFrame:
    """Development-only direction check; never selects a method post hoc."""
    if comparisons.empty:
        return pd.DataFrame([{
            "decision": "INCOMPLETE",
            "reason": "full method or required ablations are missing",
        }])
    loss = comparisons[
        comparisons.metric == "mean_changed_loss_post"
    ].copy()
    rows = []
    for _, item in loss.iterrows():
        n_pairs = int(item.n_pairs)
        minimum_wins = int(math.ceil(n_pairs / 2.0)) if n_pairs else 0
        direction_pass = bool(
            item.mean_diff_full_minus_ablation < 0.0
            and int(item.full_win_count) >= minimum_wins
        )
        if n_pairs < 3:
            decision = "INSUFFICIENT_SEEDS"
        else:
            decision = "PASS" if direction_pass else "FAIL"
        rows.append({
            "full_method": item.full_method,
            "ablation": item.ablation,
            "loss_diff_full_minus_ablation": (
                item.mean_diff_full_minus_ablation
            ),
            "full_loss_win_count": int(item.full_win_count),
...
def validate_args(args) -> None:
    ...
    if not (0.0 < args.mcglr_single_shift < 1.0):
        raise ValueError("--mcglr_single_shift must be strictly in (0,1)")
...
def run_self_tests() -> None:
    ...
    args = argparse.Namespace(
        ...
        mcglr_single_shift=0.20,
    )
    for kind in (
        "mcglr_top2", "mcglr_top1", "mcglr_single",
        "mcglr_instant_top2",
    ):
    ...
    print(
        "V17 self-test passed: official OGPM, all four controlled GLR "
        "variants, bounded loss, and weighted FedAvg."
    )
...
def main(args) -> None:
    ...
    print("=" * 112)
    print("V20: controlled MC-PrivGLR ablation under the frozen V15 protocol")
    ...
    atomic_to_csv(
        ...
        output / "v20_privacy_channel_diagnostics.csv",
    )
    ...
    atomic_to_csv(pd.DataFrame(method_registry), output / "v20_method_registry.csv")
    ...
    for fraction in args.changed_fractions:
        ...
        for seed_index, seed in enumerate(range(args.seed, args.seed + args.num_stream_seeds), start=1):
            curve_path = per_seed_dir / f"v20_curve_f{fraction_tag:04d}_s{seed}.csv"
            summary_path = per_seed_dir / f"v20_summary_f{fraction_tag:04d}_s{seed}.csv"
            decision_path = per_seed_dir / f"v20_decisions_f{fraction_tag:04d}_s{seed}.csv"
            cached = False
            ...
            for policy_index, (name, kind, epsilon) in enumerate(specs, start=1):
                ...
                policy_prefix = f"v20_f{fraction_tag:04d}_s{seed}_{policy_tag}"
    ...
    atomic_to_csv(curves_df, output / "v20_all_curves.csv")
    atomic_to_csv(summary_df, output / "v20_per_seed_summary.csv")
    if args.save_decisions:
        atomic_to_csv(
            pd.concat(all_decisions, ignore_index=True),
            output / "v20_all_decisions.csv",
        )
    ...
    ablation_pairwise = ablation_comparisons(primary, args)
    ablation_decision = ablation_gate(ablation_pairwise)
    atomic_to_csv(primary_summary, output / "v20_primary_policy_summary.csv")
    atomic_to_csv(pairwise, output / "v20_pairwise_primary.csv")
    atomic_to_csv(
        ablation_pairwise,
        output / "v20_ablation_pairwise.csv",
    )
    atomic_to_csv(
        ablation_decision,
        output / "v20_ablation_gate.csv",
    )
    leaderboard = development_leaderboard(primary, args)
    atomic_to_csv(leaderboard, output / "v20_ablation_leaderboard.csv")

    label = epsilon_label(args.primary_epsilon)
    proposed_name = f"mcglr_top2_ogpm_{label}"
    proposed_row = primary[primary.policy == proposed_name]
    report_lines = [
        "V17 MC-PrivGLR 受控消融预实验说明",
        "=" * 88,
        f"版本: {VERSION}",
        ...
        "完整方法: MC-PrivGLR-top2；消融仅移除Top-2融合、多尺度或时间累积中的一个因素",
        "闭环: 每个策略拥有独立动态模型，当前选择会改变模型、下一轮监控报告和状态转移",
        f"本次运行策略: {', '.join(name for name, _, _ in specs)}",
        "受控消融: Top-1、固定单尺度、无时间累积instant-top2",
        "边界控制: Random 与 Oracle；它们不参与组件贡献判定",
        "",
    ...
    report_lines.extend([
        "",
        "消融判定口径:",
        "1. 单种子只检查代码与方向，不据此形成论文结论。",
        "2. 至少3个paired seeds后，完整方法changed loss均值应低于每个消融。",
        "3. 同时报告changed accuracy、AUPRC、Recall@B及stable/balanced安全诊断。",
    ])
    report_path = output / "V17_MCPrivGLR_受控消融说明.txt"
    report_path.write_text("\n".join(report_lines), encoding="utf-8")
    print("\n" + "\n".join(report_lines))
    print("\nKey outputs:")
    for path in [
        report_path,
        output / "v20_primary_policy_summary.csv",
        output / "v20_ablation_pairwise.csv",
        output / "v20_ablation_gate.csv",
        output / "v20_method_registry.csv",
        output / "v20_ablation_leaderboard.csv",
        output / "v20_per_seed_summary.csv",
        output / "v20_all_curves.csv",
        output / "v20_privacy_channel_diagnostics.csv",
        output / "v20_config.json",
    ]:
        print(" ", path.resolve())
    print("\nV17 ablation leaderboard (lower changed loss is better):")
    display_columns = [
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "V17 controlled MC-PrivGLR ablation under the unchanged "
            "V15 OGPM/FedAvg protocol"
        ),
    )
    ...
    parser.add_argument("--out_dir", type=str, default=r"results\v20_mcprivglr_ablation")
    ...
    parser.add_argument(
        "--policy_subset", nargs="+", default=["core_ablation"],
        help=(
            "core_ablation, fixed_shift, ablation, paper, screen, all, or exact policy/controller tokens. "
            "core_ablation contains frozen Top-2, Top-1 and no-history instant Top-2; "
            "fixed_shift runs the predeclared single-scale diagnostic selected by --mcglr_single_shift. "
            "The legacy ablation set also contains Random/Oracle/single-scale. The paper "
    ...
    parser.add_argument(
        "--mcglr_single_shift", type=float, default=0.20,
        help=(
            "Predeclared shift used only by the single-scale ablation. The "
            "closest entry in --mcglr_shifts is selected without inspecting "
            "stream outcomes."
        ),
    )
